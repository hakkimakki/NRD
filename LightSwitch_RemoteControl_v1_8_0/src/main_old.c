...
//static struct k_work_delayable buttons_lock;
...
/*
static void buttons_lock_fn(struct k_work *work)
{
...
void button_pressed(const struct device *dev, struct gpio_callback *cb,
                    uint32_t pins)
{
  printk("Button %u pressed at %" PRIu32 "\n",pins, k_cycle_get_32());
  if (!buttons_locked){
    buttons_locked = true;
    pressed_pins = pins;
    k_work_reschedule(&buttons_debounce, K_MSEC(100)); // Debounce the Button
    //k_work_reschedule(&buttons_lock, K_MSEC(1000)); // unlock Buttons
  }
}
...
static void buttons_init()
{
  ...
  //k_work_init_delayable(&buttons_lock, buttons_lock_fn);
  // Test Wake UP
  /* Configure to generate PORT event (wakeup) on buttons press. 
  nrf_gpio_cfg_sense_set(button_1_on.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_2_on.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_3_on.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_4_on.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_1_off.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_2_off.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_3_off.pin,NRF_GPIO_PIN_SENSE_LOW);
  nrf_gpio_cfg_sense_set(button_4_off.pin,NRF_GPIO_PIN_SENSE_LOW);
...
const struct device *dt_timer0;
const struct device *dt_timer1;
const struct device *dt_rtc0;
const struct device *dt_rtc1;
const struct device *dt_rtc2;

static void lpn_app_init(){
  cons = device_get_binding(CONSOLE_LABEL);
  dt_timer0 = device_get_binding("TIMER_0");
  dt_timer1 = device_get_binding("TIMER_1");
  dt_rtc0 = device_get_binding("RTC_0");
  dt_rtc1 = device_get_binding("RTC_1");
  dt_rtc2 = device_get_binding("RTC_2");
}

static void lpn_wakeup(){
  pm_device_state_set(cons, PM_DEVICE_STATE_ACTIVE);
  //pm_device_state_set(dt_timer0,PM_DEVICE_STATE_ACTIVE);  
  //pm_device_state_set(dt_timer1,PM_DEVICE_STATE_ACTIVE);
  
  //pm_device_state_set(dt_rtc0,PM_DEVICE_STATE_ACTIVE);   
  //pm_device_state_set(dt_rtc1,PM_DEVICE_STATE_ACTIVE);   
  //pm_device_state_set(dt_rtc2,PM_DEVICE_STATE_ACTIVE);
...
static void lpn_sleep(){
    ...
    //pm_device_state_set(dt_timer0,PM_DEVICE_STATE_SUSPENDED);  
    //pm_device_state_set(dt_timer1,PM_DEVICE_STATE_SUSPENDED);
    
    //pm_device_state_set(dt_rtc0,PM_DEVICE_STATE_SUSPENDED);   
    //pm_device_state_set(dt_rtc1,PM_DEVICE_STATE_SUSPENDED);   
    //pm_device_state_set(dt_rtc2,PM_DEVICE_STATE_SUSPENDED);  
         
    //clk_nrf->TASKS_CTSTOP = true;
