...
/* The devicetree node identifier for the "switch" node. */
#define SWITCH_NODE DT_NODELABEL(switch)

#if DT_NODE_HAS_STATUS(SWITCH_NODE, okay)
#define SWITCH	DT_GPIO_LABEL(SWITCH_NODE, gpios)
#define PIN_SWITCH	DT_GPIO_PIN(SWITCH_NODE, gpios)
#if DT_PHA_HAS_CELL(SWITCH_NODE, gpios, flags)
#define FLAGS_SWITCH	DT_GPIO_FLAGS(SWITCH_NODE, gpios)
#endif
#else
/* A build error here means your board isn't set up to blink an LED. */
...
/*
 * Get Signal configuration from the devicetree signal node.
 *
 * At least a GPIO device and pin number must be provided. The 'flags'
 * cell is optional.
 */

#define SIGNAL_NODE	DT_NODELABEL(signal)

#if DT_NODE_HAS_STATUS(SIGNAL_NODE, okay)
#define SIGNAL_GPIO_LABEL	DT_GPIO_LABEL(SIGNAL_NODE, gpios)
#define SIGNAL_GPIO_PIN	DT_GPIO_PIN(SIGNAL_NODE, gpios)
#define SIGNAL_GPIO_FLAGS	(GPIO_INPUT | FLAGS_OR_ZERO(SIGNAL_NODE) | GPIO_INT_DEBOUNCE)
#else
#error "Unsupported board: signal devicetree alias is not defined"
#define SIGNAL_GPIO_LABEL	""
#define SIGNAL_GPIO_PIN	0
#define SIGNAL_GPIO_FLAGS	0
#endif

/*
 * Get Pulse configuration from the devicetree pulse node.
 *
 * At least a GPIO device and pin number must be provided. The 'flags'
 * cell is optional.
 */

#define PULSE_NODE	DT_NODELABEL(pulse)

#if DT_NODE_HAS_STATUS(PULSE_NODE, okay)
#define PULSE_GPIO_LABEL	DT_GPIO_LABEL(PULSE_NODE, gpios)
#define PULSE_GPIO_PIN	DT_GPIO_PIN(PULSE_NODE, gpios)
#define PULSE_GPIO_FLAGS	(GPIO_INPUT | FLAGS_OR_ZERO(PULSE_NODE) | GPIO_INT_DEBOUNCE)
#else
#error "Unsupported board: pulse devicetree alias is not defined"
#define PULSE_GPIO_LABEL	""
#define PULSE_GPIO_PIN	0
#define PULSE_GPIO_FLAGS	0
#endif
