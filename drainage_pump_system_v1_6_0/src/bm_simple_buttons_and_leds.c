...
const struct device *dev_led0, *dev_led1, *dev_led2, *dev_led3, *dev_pumpdrain, *dev_valvesdrain, *dev_button0;
bool led0_is_on, led1_is_on, led2_is_on, led3_is_on, pumpdrain_is_on, valvesdrain_is_on = true;
...
void bm_init_buttons(void (*button0_cb)()) {
  ...
  gpio_init_callback(&button_cb_data, button_pressed_handler, BIT(SW0_GPIO_PIN));
...
void bm_init_leds() {
  ...
  dev_pumpdrain = device_get_binding(PUMPDRAIN);
  if (dev_pumpdrain == NULL) {
    printk("Error no PUMPDRAIN found");
    return;
  }
  ret = gpio_pin_configure(dev_pumpdrain, PIN_PUMPDRAIN, GPIO_OUTPUT_ACTIVE | FLAGS_PUMPDRAIN);
  if (ret < 0) {
    printk("Error in configuration of pumpdrain pin (err %d)", ret);
    return;
  }

  dev_valvesdrain = device_get_binding(VALVESDRAIN);
  if (dev_valvesdrain == NULL) {
    printk("Error no VALVESDRAIN found");
    return;
  }
  ret = gpio_pin_configure(dev_valvesdrain, PIN_VALVESDRAIN, GPIO_OUTPUT_ACTIVE | FLAGS_VALVESDRAIN);
  if (ret < 0) {
    printk("Error in configuration of valvesdrain pin (err %d)", ret);
    return;
  }
...
void bm_pumpdrain_set(bool state) {
  gpio_pin_set(dev_pumpdrain, PIN_PUMPDRAIN, (int)state);
  pumpdrain_is_on = !state;
}

/* Get PUMPDRAIN */
bool bm_pumpdrain_get() {
  return pumpdrain_is_on;
}

/* Set VALVESDRAIN  */
void bm_valvesdrain_set(bool state) {
  gpio_pin_set(dev_valvesdrain, PIN_VALVESDRAIN, (int)state);
  valvesdrain_is_on = !state;
}

/* Get PUMPDRAIN */
bool bm_valvesdrain_get() {
  return valvesdrain_is_on;
}
